...
def parse_args():
    ...
    parser.add_argument('--file_name', type=str, default= "./results_dump/answer_dump/eval__answer_toxigen_vicuna_COV_pns_use_pns_seed_2_top_18_heads_alpha_5.0_fold_0_special.csv", help='Path to the CSV file to process')
    parser.add_argument('--model', type=str, default = "COV_pns_use_pns", help='Model name to use')
...
def main():
    ...
    if pd.isna(start_idx):
        print("All rows already processed :white_check_mark:")
        return
    ...
    for i in tqdm(range(start_idx, len(df))):
        ...
        if (i + 1) % 50 == 0 or (i + 1) == len(df):
            ...
            print(f":heavy_check_mark: Saved progress at row {i + 1}")
    print(":white_check_mark: All rows processed and saved.")
