...
def load_text_df(dataset_name: str) -> pd.DataFrame:
    """
    Mirror the dataset loading logic from validate_2fold_toxic.py,
    but as a helper so we can separately load:
      - the evaluation dataset (args.dataset_name)
      - the direction/top-heads dataset (args.dir_dataset_name)
    """
    if dataset_name == "toxigen":
        _ = load_dataset("json", data_files="../../dataset/toxiGen.json")["train"]
        df = pd.read_csv(f'./TruthfulQA/{dataset_name}.csv')
    elif dataset_name == "hate":
        _ = load_dataset("json", data_files="../../dataset/implicitHate.json")["train"]
        df = pd.read_csv(f'./TruthfulQA/shuffled_{dataset_name}.csv')
...
def load_headwise_activations(model_name: str, dataset_name: str, num_heads: int):
    """
    Load head-wise activations and labels for a given (model, dataset),
    reshaping to (n, l, h, d).
    """
    head_wise_activations = np.load(
        f"/work/hdd/bcxt/yian3/toxic/features/{model_name}_{dataset_name}_head_wise.npy"
    )
    head_wise_activations = rearrange(
        head_wise_activations, 'b l (h d) -> b l h d', h=num_heads
    )
    labels = np.load(
        f"/work/hdd/bcxt/yian3/toxic/features/{model_name}_{dataset_name}_labels.npy"
    )
...
def main():
    ...
    parser.add_argument('--dataset_name', type=str, default='toxigen_vicuna', help='dataset for evaluation (OOD relative to direction dataset)')
    # dir_dataset_name: where COM directions and top_heads are computed
    parser.add_argument(
        '--dir_dataset_name',
        type=str,
        default=None,
        help='dataset used to compute COM directions and top_heads; if None, fall back to dataset_name (in-distribution)',
    )

    parser.add_argument('--num_heads', type=int, default=48, help='K, number of top heads to intervene on')
    parser.add_argument('--heads_path', type=str,
                        default=None,
                        help='Optional: explicit path to selected heads numpy file (OOD version will otherwise infer path from dir_dataset_name)')
    ...
    # direction dataset defaults to evaluation dataset if not specified (ID case)
    dir_dataset_name = args.dir_dataset_name or args.dataset_name
    ...
    # ------------------------------------------------------------------
    # Evaluation dataset: used only for CV splits and evaluation
    # ------------------------------------------------------------------
    eval_df = load_text_df(args.dataset_name)
    eval_fold_idxs = np.array_split(np.arange(len(eval_df)), args.num_fold)

    # ------------------------------------------------------------------
    # Create model (same as original script)
    # ------------------------------------------------------------------
    if args.model_name in ['llama3_8B']:
        model_name = 'meta-llama/Meta-Llama-3-8B'
    elif args.model_name in ['vicuna_13B']:
    ...
    # For now, only support LLaMA-style models in this OOD script, mirroring validate_2fold_toxic.py
    ...
    try:
        ...
        num_layers = 12  # default for tiny_gpt2-style models
        num_heads = 12

    # ------------------------------------------------------------------
    # Direction dataset: where COM directions and top_heads are computed
    # ------------------------------------------------------------------
    dir_df = load_text_df(dir_dataset_name)

    head_wise_activations_dir, labels_dir = load_headwise_activations(
        args.model_name, dir_dataset_name, num_heads
    )
    tuning_activations, tuning_labels = load_headwise_activations(
        args.model_name, dir_dataset_name, num_heads
    )

    n_dir, l_dir, h_dir, d_dir = head_wise_activations_dir.shape
    input_dim_dir = l_dir * h_dir * d_dir

    # Train VAE and extract mu for direction dataset only (once)
    _, _, head_wise_c_dir = train_vae_and_extract_mu(
        head_wise_activations_dir,
        labels_dir,
        input_dim_dir,
        z_dim=32,
        h_dim1=128,
        ...
        lr=1e-3,
        vae_epochs=10,
        dataset_name=dir_dataset_name,
        model_name=args.model_name,
        mode='valid',
        device='cuda',
    )
    print("Direction-dataset head_wise_c size", head_wise_c_dir.size(), args.model_name)

    (
        separated_head_wise_activations_dir,
        separated_labels_dir,
        separated_head_wise_c_dir,
        idxs_to_split_at_dir,
    ) = get_activations(
        labels_dir,
        head_wise_activations_dir,
        head_wise_c_dir,
    ...
    # Create separate folds for direction dataset
    dir_fold_idxs = np.array_split(np.arange(len(separated_labels_dir)), args.num_fold)

    # ------------------------------------------------------------------
    # Run k-fold cross validation on EVAL dataset, using directions from DIR dataset
    # ------------------------------------------------------------------
    results = []
    for i in range(args.num_fold):
        # training indices for direction dataset (for computing COM, top_heads)
        dir_train_idxs = np.concatenate(
            [dir_fold_idxs[j] for j in range(args.num_fold) if j != i]
        )
        dir_val_idxs = None  # you can change this to use a held-out fold if desired

        print(f"Running fold {i}")

        # Create necessary directories with OOD indicator
        # Add OOD indicator and include both direction dataset and test dataset in path
        ood_suffix = f"_ood_dir_{dir_dataset_name}_test_{args.dataset_name}" if dir_dataset_name != args.dataset_name else f"_ood_dir_{dir_dataset_name}"
        os.makedirs(f'results_dump/answer_dump/{args.dataset_name}{ood_suffix}/{orig_model}', exist_ok=True)
        os.makedirs(f'results_dump/summary_dump/{args.dataset_name}{ood_suffix}/{orig_model}', exist_ok=True)

        # ------------------------------------------------------------------
        # Get COM directions from DIRECTION dataset
        # ------------------------------------------------------------------
        if args.use_center_of_mass:
            ...
            direct_path = f"/work/hdd/bcxt/yian3/toxic/features/{args.model_name}_{dir_dataset_name}_special_direction.npy"
            if os.path.exists(direct_path):
                ...
                com_directions = get_special_directions(
                    num_layers,
                    num_heads,
                    dir_train_idxs,
                    dir_val_idxs,
                ...
                np.save(direct_path, com_directions)
        elif args.use_mat_direction:
            com_directions = get_matrix_directions(
                num_layers,
                num_heads,
                dir_train_idxs,
                dir_val_idxs,
        ...
        print("Finished computing com_directions of shape", None if com_directions is None else com_directions.shape)

        # ------------------------------------------------------------------
        # Get top heads from DIRECTION dataset
        # ------------------------------------------------------------------
        if args.mode == 'get_top_heads':
            if args.use_pns:
                top_heads, pns_scores = get_top_heads_pns(
                    dir_train_idxs,
                    dir_val_idxs,
                    separated_head_wise_activations_dir,
                    separated_labels_dir,
                    ...
                    num_layers,
                    num_heads,
                    num_to_intervene=args.num_heads,
                    lambda_reg=1e-4,
                    sigma_sq=1.0,
                    seed=42,
                ...
                np.save(
                    f'/work/hdd/bcxt/yian3/toxic/features/heads/{args.use_pns}_{args.model_name}_{dir_dataset_name}_seed_{args.seed}_top_{args.num_heads}_heads_fold_{i}_pns_scores.npy',
                    pns_scores,
                )
                probes = None
            else:
            ...
            np.save(
                f'/work/hdd/bcxt/yian3/toxic/features/heads/{args.use_pns}_{args.model_name}_{dir_dataset_name}_seed_{args.seed}_top_{args.num_heads}_heads_fold_{i}.npy',
                top_heads,
            )
            print(
                f"top heads saved to /work/hdd/bcxt/yian3/toxic/features/heads/{args.use_pns}_{args.model_name}_{dir_dataset_name}_seed_{args.seed}_top_{args.num_heads}_heads_fold_{i}.npy"
            )
            # For pure "get_top_heads" runs, we don't evaluate, just continue
            continue

        # When not in "get_top_heads" mode, we either load heads for the
        # direction dataset or compute them if missing.
        if args.heads_path is not None:
            heads_path = args.heads_path
        else:
            heads_path = (
                f'/work/hdd/bcxt/yian3/toxic/features/heads/'
                f'{args.use_pns}_{args.model_name}_{dir_dataset_name}_seed_{args.seed}_'
                f'top_{args.num_heads}_heads_alpha_{args.alpha}_fold_{i}_top_heads.npy'
            )

        if os.path.exists(heads_path):
            selected_heads = np.load(heads_path)
            top_heads = (
                selected_heads[: args.num_heads]
                if len(selected_heads) > args.num_heads
                else selected_heads
            )
            probes = None
        else:
            if args.use_pns:
                ...
                np.save(
                    f'/work/hdd/bcxt/yian3/toxic/features/heads/{args.use_pns}_{args.model_name}_{dir_dataset_name}_seed_{args.seed}_top_{args.num_heads}_heads_alpha_{args.alpha}_fold_{i}_pns_scores.npy',
                    pns_scores,
                )
                probes = None
            else:
            ...
            np.save(heads_path, top_heads)

        print("Heads intervened (direction dataset): ", top_heads)
        ...
        interventions = get_interventions_dict(
            ...
            args.use_random_dir,
            args.use_mat_direction,
            args.use_special_direction,
            com_directions,
        )
        print("Finished computing interventions dict")

        # ------------------------------------------------------------------
        # Define intervention function (same as original)
        # ------------------------------------------------------------------
            ...
            for head, direction, proj_val_std in interventions[layer_name]:
                if len(direction.shape) == 2:  # use_mat_direction or use_special_direction
                    ...
                    if start_edit_location == 'lt':
                        ...
                        if prompt_encoding is None:
                            direction_to_add = torch.einsum(
                                'bij,jk->bik',
                                head_output[:, start_edit_location:, head, :],
                                direction.T,
                            )
                            direction_to_add = direction_to_add / torch.linalg.norm(
                                direction_to_add, axis=2
                            )[:, :, None]
                        else:
                            direction_to_add = prompt_encoding @ direction.T  # 1 x 128
                            direction_to_add = direction_to_add.unsqueeze(1).repeat(
                                1, head_output.shape[1] - start_edit_location, 1
                            )
                            direction_to_add = direction_to_add / torch.linalg.norm(
                                direction_to_add, axis=2
                            )[:, :, None]

                        proj_vals = torch.einsum(
                            'Bk,bik->Bbi', activations, direction_to_add
                        )
                        proj_val_std = torch.std(proj_vals, axis=0)[:, :, None]

                    proj_val_std = torch.Tensor(proj_val_std).to(
                        head_output.device.index
                    )
                    if start_edit_location == 'lt':
                        head_output[:, -1, head, :] += (
                    ...
                    direction_to_add = torch.FloatTensor(direction).to(
                        head_output.device.index
                    )
                    if start_edit_location == 'lt':
                        head_output[:, -1, head, :] += (
        ...
        # ------------------------------------------------------------------
        # Build output filenames and evaluation paths (based on EVAL dataset)
        # ------------------------------------------------------------------
        ...
        if args.dataset_name == 'hate':
            ...
            output_path = f'results_dump/answer_dump/{args.dataset_name}{ood_suffix}/{orig_model}/{output_filename}.csv'
            summary_path = f'results_dump/summary_dump/{args.dataset_name}{ood_suffix}/{orig_model}/{output_filename}.csv'
        elif args.dataset_name == 'toxigen':
            input_path = f'splits/{args.dataset_name}_fold_{i}_test_seed_{args.seed}.csv'
            output_path = f'results_dump/answer_dump/{args.dataset_name}{ood_suffix}/{orig_model}/{prefix}_answer_no_inst_{output_filename}.csv'
            summary_path = f'results_dump/summary_dump/{args.dataset_name}{ood_suffix}/{orig_model}/{output_filename}.csv'
        elif args.dataset_name in ['toxigen_vicuna', 'hate_vicuna', 'paradetox']:
            input_path = f'splits/{args.dataset_name}_fold_{i}_test_seed_{args.seed}.csv'
            output_path = f'results_dump/answer_dump/{args.dataset_name}{ood_suffix}/{orig_model}/{prefix}_answer_no_inst_{output_filename}.csv'
            summary_path = f'results_dump/summary_dump/{args.dataset_name}{ood_suffix}/{orig_model}/{prefix}_summary_no_inst_{output_filename}.csv'
        else:
            raise ValueError(f"Unsupported evaluation dataset_name: {args.dataset_name}")
    ...
    if args.mode != 'get_top_heads' and len(results) > 0:
        results = np.array(results)
        final = results.mean(axis=0)
        print("Final mean results across folds:", final)
