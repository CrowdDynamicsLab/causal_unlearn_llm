...
parser.add_argument('-d', '--dataset', type=str, default='toxigen',choices=['yelp', 'amazon', 'tripadvisor', 'toxigen'])
...
if moniker == 'toxigen':
    full_dat = pd.read_csv('../../causal_to_concept/llms/TruthfulQA/toxigen.csv')
    full_dat = full_dat.rename(columns={'label':'y', 'text':'review_text'})
    data = full_dat
    texts = list(data['review_text'])
...
print("top_feature_idx", top_feature_idx)
...
for step in range(flags.steps):
    ...
    if step % 100 == 0:
        ...
        classes = torch.unique(torch.tensor(train_y))
        means_by_class = []
        for cls in classes:
            mask = (torch.tensor(train_y).view(-1) == cls)
            class_mean = torch.tensor(train_features)[mask].mean(dim=0)
            means_by_class.append(class_mean)

        means_by_class = torch.stack(means_by_class)  # shape: [num_classes, D]
        print("means_by_class", means_by_class)
